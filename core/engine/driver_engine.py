...
from core.data.constants import SCREENSHOTS
from core.data.enums import Type
...
log = Logfire(name='driver-engine')
...
class DriverEngine(DriverManager):
    ...
    def get_web(self, web_link: str, maximize_window=True) -> None:
        ...
        log.fire.info(f'webdriver used: \n{self.driver} \n started: \n {web_link}')

        if maximize_window:
            log.fire.info('maximize window enabled')
    ...
    def get_element(self, name: str, seconds=10) -> webdriver:
        ...
        try:
            if element_type in Type.__members__:
                log.fire.info(f'element: {output}')
